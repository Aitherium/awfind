...
**[Docs](https://aitherium.github.io/awfind/)**  ·  [Source](https://github.com/Aitherium/awfind)  ·  `pip install awfind`  ·  [The Aither World](https://aitherium.github.io/)

> **The Aither World** is an operating system for agents — a Linux you can hand to one, the runtimes it works in, and the tools it works with. [awnix](https://github.com/Aitherium/awnix) is the Linux underneath it; **awfind** is one of its 55 bricks — each installs on its own, runs offline, and needs no account.
>
...
awfind doctor             # what this machine resolves, and why
...
|---|---|---|
| 1 | `--url` | `--token` |
| 2 | `AWFIND_URL` | `AWFIND_TOKEN` |
| 3 | `~/.aither/awfind.json` (move it with `AWFIND_CONFIG`) | same file |
| 4 | an error that names rungs 1-3 | none — unauthenticated is legal |

There is **no rung 5**. No default endpoint is baked in, and none ever will be:
a search client that quietly falls back to some address is sending your queries
somewhere you did not choose, and one that ships its author's address is useless
to everyone else. The test suite asserts that no internal address appears as a
URL anywhere in the package source, because behaviour cannot catch a baked
default — it works perfectly, on exactly one machine.

What the fourth rung prints is the point of the whole arrangement:

```
awfind: no service URL. Tried, in order:
  1. --url (not given)
  2. AWFIND_URL (not set)
  3. /home/you/.aither/awfind.json (no file)
Fix it once, for every future run:
  awfind config set --url https://<host>:<port>   (writes /home/you/.aither/awfind.json)
or for this shell only:
  export AWFIND_URL=https://<host>:<port>
```

A message that names two of the three rungs and no file at all — which is what
this used to print — leaves the durable fix undiscoverable from the failure.
The environment variable is not a fix; it dies with the shell, and it is absent
from every cron entry, service unit and agent subprocess that needed it.

**A broken config is reported, not swallowed.** A file that exists but is
unparseable, is not an object, or holds a URL with no scheme raises with the
path and the reason. Collapsed into the same "nothing is configured" message,
a typo would send you to write the config you already wrote.

### TLS on a private CA

`SSL_CERT_FILE`, then `REQUESTS_CA_BUNDLE`, then the config file's `ca_bundle`,
then the library's own trust store. Those two variable names are not invented
here — OpenSSL reads the first and `requests` the second — so a machine already
set up for a private CA works without being told twice. Verification is never
turned off: no rung can resolve to "do not verify", because an unverified search
client is one an attacker can answer for.
...
| **awfind** _(you are here)_ | A portable search client — query, results, ranking | [docs](https://aitherium.github.io/awfind/) |
...
<div id="aither-constellation" data-self="awfind"></div>
